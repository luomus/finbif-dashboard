...
occurrence_plot <-
  reactive({
    ...
    future_promise(
      ...
      seed = TRUE,
      globals = list(translator = translator, req = req, lang = input$lang),
...
municipality_map <-
  reactive({
    ...
    level <- switch(
      ...
      paste0("maakunta_name_", input$lang)
    )
    ...
    future_promise(
      {
        ...
        gg <-
          ...
          scale_fill_viridis_c(
            label_comma(big.mark = big_mark, decimal.mark = dec_mark)
          )
...
projects <-
  reactive({
    ...
    names(choices) <- c(
      ...
      translator$t(c("All Project", names(collections$cit_sci_projects)))
    )
...
project_species_plot <-
  reactive({
    ...
    req <-
      ...
      req_url_query(projects = input$projects, lang = input$lang)
  ...
  bindCache(input$projects, input$lang, cache = "session") |>
  bindEvent(input$project, input$lang)
