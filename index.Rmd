...
### `r textOutput("datareq_disclosure_plot_title", inline = TRUE)` {#disclosure-datareq-plot}

```{r datareq_disclosure_plot_title_render, context = "server"}
datareq_disclosure_plot_title <-
  reactive({
    translator$set_translation_language(sanitise_lang(input$lang))
    translator$t("Data Requests by Disclosure")
  })|>
  bindCache(input$lang, cache = "session") |>
  bindEvent(input$lang)

output$datareq_disclosure_plot_title <- renderText(datareq_disclosure_plot_title())
```


```{r datareq_disclosure_plot_output, context = "render", echo = FALSE}
fillCol(plotlyOutput("datareq_disclosure_plot"))
```

```{r datareq_disclosure_plot_render, context = "server"}
datareq_disclosure_plot <-
  reactive({

    req <-
      request(pyha_api) |>
      req_url_path("api/statistics/partyInvolvementCounts")|>
      req_url_query(
        year = input$datareq_year,
        lang = sanitise_lang(input$lang)
      )
    ...
    future_promise(
      {
        ...
        count <- translator$t("Data Requests")
        disclosure <- translator$t("Disclosure")
        ...
        res <- structure(
          as.data.frame(do.call(rbind, lapply(res[["results"]], unlist))),
          names = c(disclosure, count)
        )

        gg <-
          ggplot(res) +
          aes(.data[[disclosure]], .data[[count]]) +
          geom_bar(stat = "identity")
...
output$datareq_disclosure_plot <- renderPlotly({
