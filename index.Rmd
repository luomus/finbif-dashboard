...
onRestored({
  function(state) {
    runjs('
      ...
      Shiny.setInputValue("datreqinstitutions_link",  $.urlParam("datreqinstitutions"));
    ')
  }
})
...
observe(
  updateSelectInput(
    session,
    "datreqinstitutions",
    selected = input[["datreqinstitutionss_link"]]
  )
) |>
bindEvent(input$datreqinstitutions_link, ignoreInit = TRUE)
...
`r textOutput("datreq_title")` {#datreq-page}
================================================================================

```{r datreq_title_render, context = "server"}
datreq_title <-
  reactive({
    translator$set_translation_language(sanitise_lang(input$lang))
...
output$datreq_title <- renderText(datreq_title())
```
...
```{r datreq_page_inputs, context = "render", echo = FALSE}
uiOutput("datreqinstitutions")
```

```{r datreqinstitution_render, context = "server"}
datreqinstitutions <-
  reactive({
...
output$datreqinstitutions <- renderUI(datreqinstitutions())
```
...
selectInput(
  "datreqinstitutions", NULL, choices = c("", "NULL", collections$sources)
)
```
...
```{r datareq_plot_output, context = "render", echo = FALSE}
fillCol(plotlyOutput("datareq_plot"))
```

```{r datareq_plot_render, context = "server"}
datareq_plot <-
  reactive({

    req <-
      request("https://staging-pyha.laji.fi") |>
...
output$datareq_plot <- renderPlotly({
