...
setBookmarkExclude(
  c(
    "reset1", "reset2", "reset3", "reset4", "lang_link",
    "occurrence_data_plot_active_link", "collections_data_plot_active_link",
    "citsci_data_plot_active_link", ".clientValue-default-plotlyCrosstalkOpts",
    "plotly_afterplot-A", "plotly_hover-A", "plotly_relayout-A",
    "quality_table_cell_clicked", "quality_table_cells_selected",
    "quality_table_columns_selected", "quality_table_rows_all",
    "quality_table_rows_current", "quality_table_rows_selected",
    "quality_table_search", "quality_table_state", "info_waiter_hidden",
    "record_count_waiter_hidden", "species_count_waiter_hidden",
    "collection_count_waiter_hidden", "quality_table_waiter_hidden",
    "occurrence_plot_waiter_hidden", "species_plot_waiter_hidden",
    "annotations_plot_waiter_hidden", "datasets_plot_waiter_hidden",
    "municipality_map_waiter_hidden", "specimen_count_waiter_hidden",
    "digitised_count_waiter_hidden", "digitised_percent_waiter_hidden",
    "imaged_count_waiter_hidden", "imaged_percent_waiter_hidden",
    "collection_plot_waiter_hidden", "progress_plot_waiter_hidden",
    "specimen_map_waiter_hidden", "project_count_waiter_hidden",
    "project_species_plot_waiter_hidden", "project_users_plot_waiter_hidden",
    "occurrence_cit_plot_waiter_hidden"
  )
)
...
$("body").on(
  ...
  function(e) {
    ...
    navigator.clipboard
      .writeText(url + hash)
      .then(() => {
        alert("A link to the current dashboard view has been copied to the clipboard");
      })
      .catch(() => {
        alert("Error");
      });
...
$("body").on(
  ...
  function(e) {
    Shiny.setInputValue(
      "occurrence_data_plot_active_link",
...
$("body").on(
  ...
  function(e) {
    Shiny.setInputValue(
      "collections_data_plot_active_link",
      $(e.target).parent().index()
    );
  }
)
...
Occurrence Data {#occurrence-page}
================================================================================
...
```{r info_output, context = "render", echo = FALSE}
hr()
uiOutput("info")
```
...
```{r info_output_render, context = "server"}
info <-
  reactive({
    ...
    tagList(
      bs_embed_popover(
        shiny_iconlink(), 
        content = translator$t("Select filter"),
        placement = "right"
      ),
...
selectInput("occurrence_data_plot_active", NULL, choices = 0:3)
selectInput("collections_data_plot_active", NULL, choices = 0:1)
selectInput("citsci_data_plot_active", NULL, choices = 0:2)
```
...
observeEvent(
  input$occurrence_data_plot_active_link,
  {
    updateSelectInput(
      session, 
      "occurrence_data_plot_active",
      selected = input[["occurrence_data_plot_active_link"]]
    )
  }
)
...
observe({
  ...
  addCssClass(
    ...
    selector = sprintf(
      ...
      as.integer(input$occurrence_data_plot_active) + 1L
    )
  )
...
Specimen Collections/Digitisation {#specimen-collection-page}
================================================================================
...
observeEvent(
  input$collections_data_plot_active_link,
  {
    updateSelectInput(
      session, 
      "collections_data_plot_active",
      selected = input[["collections_data_plot_active_link"]]
    )
  }
)
...
observe({
  ...
  addCssClass(
    ...
    selector = sprintf(
      ...
      as.integer(input$collections_data_plot_active) + 1L
    )
  )
...
Specimens Map {#specimen-map-page}
================================================================================
...
observeEvent(
  input$citsci_data_plot_active_link,
  {
    updateSelectInput(
      session, 
      "citsci_data_plot_active",
      selected = input[["citsci_data_plot_active_link"]]
    )
  }
)
...
observe({
  ...
  addCssClass(
    ...
    selector = sprintf(
      ...
      as.integer(input$citsci_data_plot_active) + 1L
    )
  )
