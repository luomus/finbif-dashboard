...
### Collections {#collections-occ-plot}

```{r collection_occ_plot_output, context = "render", echo = FALSE}
plotlyOutput("collection_occ_plot")
```

```{r collection_occ_plot_render, context = "server"}
collection_occ_plot <-
  reactive({

    req <-
      req_url_path(req, "collections-plot") |>
      req_url_query(
        restriction = input$restriction,
        taxa = input$taxa, 
        source = input$source
...
output$collection_occ_plot <- renderPlotly({
